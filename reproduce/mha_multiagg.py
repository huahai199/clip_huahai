...
EVAL_START_LOSS   = 1.0
SAVE_DIR          = "./checkpoints_joint/exp1_mha_multiagg"
USE_AMP           = True
...
class MultiHeadAttentionPooling(nn.Module):
    def __init__(self, hidden_size, num_heads=8):
        ...
        self.agg_queries = nn.Parameter(torch.randn(4, hidden_size) * 0.02)  # N个查询向量
        self.num_queries = 4
    ...
    def forward(self, hidden_states, mask=None):
        ...
        Q = self.q_proj(self.agg_queries).unsqueeze(0).expand(B, -1, -1)  # [B, N, D]
        ...
        out = (attn @ V).transpose(1, 2).reshape(B, N, -1)                # [B, N, D]
        out = self.out_proj(out)
        out = self.norm(out).mean(dim=1)                                   # [B, D] 多查询取均值
        return out
...
def train():
    ...
    print("=" * 55)
    print("  MHA Pooling (4 queries) + Patch 融合残差")
    print("=" * 55)
