"""
Naive文本 + Patch图像: 均值池化+单Linear + patch融合残差
"""
...
EVAL_START_LOSS   = 1.0
SAVE_DIR          = "./checkpoints_joint/exp1_naive_patch"
USE_AMP           = True
...
class NaivePatch(nn.Module):
    """均值池化 + 单Linear + patch融合残差"""
    ...
    def __init__(self, qwen_name, clip_name, embed_dim=768):
        ...
        self.txt_proj = nn.Linear(qwen_dim, embed_dim)  # 单 Linear
...
def train():
    ...
    print("=" * 55)
    print("  Naive文本 + Patch图像: 均值池化+单Linear + patch融合残差")
    print("=" * 55)
    ...
    model = NaivePatch(QWEN_NAME, CLIP_NAME, EMBED_DIM).to("cuda")
